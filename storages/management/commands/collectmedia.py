...
class Command(NoArgsCommand):
    ...
    help = """
    Copies files from MEDIA_ROOT into the configured DEFAULT_FILE_STORAGE.
    """
    ...
    def store(self):
        ...
        for path, media in finder.list(self.ignore_patterns):
            # Prefix the relative path if the source storage specified it.
            # TODO: FileSystemStorage doesn't specify a prefix, so I think
            # this block is useless.
            if getattr(media, 'prefix', None):
                prefixed_path = os.path.join(media.prefix, path)
            else:
                prefixed_path = path

            if prefixed_path not in found_files:
                found_files[prefixed_path] = (media, path)
                self.copy_file(path, prefixed_path, media)
        ...
        if self.post_process and hasattr(self.storage, 'post_process'):
            ...
            for original_path, processed_path, processed in processor:
                if isinstance(processed, Exception):
                    self.stderr.write(
                        "Post-processing '{0}' failed!".format(original_path))
                    # Add a blank line before the traceback. Otherwise it's
                    # too easy to miss the relevant part of the error message.
                    self.stderr.write("")
                    raise processed
                if processed:
                    self.log("Post-processed '{0}' as '{1}'".format(
                        original_path, processed_path), level=1)
                    self.post_processed_files.append(original_path)
                else:
                    self.log("Skipped post-processing '{0}'".format(
                        original_path))
    ...
    def handle_noargs(self, **options):
        ...
        message = ['\n']
        if self.dry_run:
            message.append('You have activated the --dry-run option,'
                           'so no files will be modified.\n\n')

        message.append(
            'You have requested to copy media files '
            'from the MEDIA_ROOT folder'
        ...
        if self.clear:
            message.append(
                ...
                '{0} backend!\n'
                'This includes any static files that have been collected '
                'into that backend!\n\n'.format(
            ...
            message.append(
                'This will overwrite existing files that '
        ...
        post_proc_cnt = len(stored['post_processed'])

        if self.verbosity >= 1:
            template = """\n{modified_count} {identifier} copied
                to {destination}{unmodified}{post_processed}.\n
                """
            summary = template.format(**{
                'modified_count': modified_count,
                'identifier': 'media file' + (
                    '' if modified_count == 1 else 's'),
                'destination': settings.DEFAULT_FILE_STORAGE,
                'unmodified': (', {0} unmodified'.format(
                    unmodified_count) if stored['unmodified'] else ''),
                'post_processed': (
                    ', {0} post-processed'.format(
                        post_proc_cnt) if stored['post_processed'] else ''
                ),
            })
    ...
    def clear_dir(self, path):
        """
        Deletes from the destination storage
        backend all files in the given relative path.
        """
        dirs, files = self.storage.listdir(path)
        for f in files:
            fpath = os.path.join(path, f)
            if self.dry_run:
                self.log("Pretending to delete '{0}'".format(
    ...
    def delete_file(self, path, prefixed_path, source_storage):
        ...
        if self.storage.exists(prefixed_path):
            try:
                ...
                target_last_modified = self.storage.modified_time(
                    prefixed_path)
            except (OSError, NotImplementedError, AttributeError):
                # The storage doesn't support
                # ``modified_time``, or otherwise failed.
                pass
            else:
                try:
                    ...
                    # Do not delete the target file if it was
                    # modified more recently than the source.
                    # Avoid sub-second precision (see #14665, #19540).
                    if (target_last_modified.replace(microsecond=0)
                            >= source_last_modified.replace(microsecond=0)):
                        if prefixed_path not in self.unmodified_files:
                            self.unmodified_files.append(prefixed_path)
                        # Returning False here tells copy_file()
                        # to skip the file.
                        return False
            # The existing file is newer in MEDIA_ROOT
            # than it is in the storage
    ...
    def copy_file(self, path, prefixed_path, source_storage):
        ...
        if prefixed_path in self.copied_files:
            return self.log(
                "Skipping '{0}' (already copied earlier)".format(path))
...
class FileSystemMediaFinder(FileSystemFinder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.storages[settings.MEDIA_ROOT] = FileSystemStorage(
